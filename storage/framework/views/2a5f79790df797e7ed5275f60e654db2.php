...
<a href="<?php echo e(route('admin.polls.create')); ?>" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium">+ Add Poll</a>
</div>
</div>
...
<form method="GET" action="<?php echo e(route('admin.polls.index')); ?>" class="flex items-center gap-4">
...
<td class="px-4 py-3 text-slate-600 dark:text-white text-sm"><?php echo e($poll->id); ?></td>
<td class="px-4 py-3">
<div class="flex items-center justify-between">
<span class="text-slate-800 dark:text-white text-sm font-medium"><?php echo e($poll->question); ?></span>
<a href="<?php echo e(route('admin.polls.results', $poll)); ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs font-medium">View Results</a>
</div>
</td>
<td class="px-4 py-3 text-slate-600 dark:text-white text-sm"><?php echo e(ucfirst($poll->language)); ?></td>
<td class="px-4 py-3 text-slate-600 dark:text-white text-sm"><?php echo e(ucfirst($poll->vote_permission ?? 'All')); ?></td>
<td class="px-4 py-3">
<?php if($poll->is_active): ?>
<button onclick="toggleStatus(<?php echo e($poll->id); ?>)" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs font-medium">Active</button>
<?php else: ?>
<button onclick="toggleStatus(<?php echo e($poll->id); ?>)" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-xs font-medium">Inactive</button>
<?php endif; ?>
</td>
<td class="px-4 py-3 text-slate-600 dark:text-white text-sm"><?php echo e($poll->date_added->format('Y-m-d / H:i')); ?></td>
<td class="px-4 py-3">
<select class="border border-slate-300 dark:border-[#324d67] rounded px-2 py-1 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white text-xs" onchange="handleOptionChange(this, <?php echo e($poll->id); ?>)">
...
<!-- Pagination -->
<?php if($polls->hasPages()): ?>
<div class="mt-6 flex items-center justify-between">
<div class="text-slate-600 dark:text-[#92adc9] text-sm">
Showing <?php echo e($polls->firstItem()); ?> to <?php echo e($polls->lastItem()); ?> of <?php echo e($polls->total()); ?> entries
</div>
<div class="flex items-center gap-2">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Laravel Projects\Multi Author Blog\resources\views\admin\polls\index.blade.php ENDPATH**/ ?>