...
<h1 class="text-slate-800 dark:text-white text-4xl font-black leading-tight tracking-[-0.033em]"><?php echo e($page->title); ?></h1>
<p class="text-slate-500 dark:text-[#92adc9] text-base font-normal leading-normal">Page preview and details.</p>
</div>
<div class="flex items-center gap-4">
<a href="<?php echo e(route('admin.pages.edit', $page)); ?>" class="flex items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em]">
...
<div class="mt-1">
<span class="inline-flex items-center justify-center rounded-full <?php echo e($page->is_published ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300'); ?> px-2.5 py-0.5 text-xs font-medium">
...
<div class="mt-1 text-slate-800 dark:text-white text-sm"><?php echo e($page->slug); ?></div>
</div>

<?php if($page->meta_description): ?>
<div>
<label class="text-slate-600 dark:text-[#92adc9] text-sm font-medium">Meta Description</label>
<div class="mt-1 text-slate-800 dark:text-white text-sm"><?php echo e($page->meta_description); ?></div>
</div>
<?php endif; ?>

<div>
<label class="text-slate-600 dark:text-[#92adc9] text-sm font-medium">Created</label>
<div class="mt-1 text-slate-800 dark:text-white text-sm"><?php echo e($page->created_at->format('M d, Y \a\t g:i A')); ?></div>
</div>

<div>
<label class="text-slate-600 dark:text-[#92adc9] text-sm font-medium">Last Updated</label>
<div class="mt-1 text-slate-800 dark:text-white text-sm"><?php echo e($page->updated_at->format('M d, Y \a\t g:i A')); ?></div>
</div>
</div>

<div class="mt-6 pt-6 border-t border-slate-200 dark:border-[#324d67]">
<div class="flex gap-2">
<a href="<?php echo e(route('admin.pages.edit', $page)); ?>" class="flex-1 flex items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em]">
<span class="truncate">Edit Page</span>
</a>
<form method="POST" action="<?php echo e(route('admin.pages.destroy', $page)); ?>" class="flex-1">
<?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Laravel Projects\Multi Author Blog\resources\views\admin\pages\show.blade.php ENDPATH**/ ?>