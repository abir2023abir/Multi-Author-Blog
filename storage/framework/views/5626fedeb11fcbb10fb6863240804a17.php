...
<div class="relative">
<input type="checkbox" id="darkModeToggle" class="sr-only" <?php echo e(($settings['dark_mode'] ?? '0') == '1' ? 'checked' : ''); ?>>
<label for="darkModeToggle" class="flex items-center cursor-pointer">
<div class="relative">
<div class="w-12 h-6 bg-gray-300 dark:bg-gray-600 rounded-full shadow-inner transition-colors duration-200 <?php echo e(($settings['dark_mode'] ?? '0') == '1' ? 'bg-green-500' : ''); ?>"></div>
<div class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 transform <?php echo e(($settings['dark_mode'] ?? '0') == '1' ? 'translate-x-6' : ''); ?>"></div>
</div>
</label>
</div>
</div>
...
<div class="relative rounded-lg border border-slate-200 dark:border-[#324d67] bg-white dark:bg-[#111a22] overflow-hidden <?php echo e($theme['active'] ? 'ring-2 ring-primary' : ''); ?>">
<!-- Theme Preview -->
<div class="relative h-64 bg-gray-100 dark:bg-gray-800 overflow-hidden">
<?php if($theme['slug'] === 'magazine'): ?>
<!-- Magazine Preview -->
...
<!-- Theme Info -->
<div class="p-4">
<h3 class="text-slate-800 dark:text-white text-lg font-bold mb-2"><?php echo e($theme['name']); ?></h3>
<p class="text-slate-500 dark:text-[#92adc9] text-sm mb-4"><?php echo e($theme['description']); ?></p>
<?php if($theme['active']): ?>
...
<?php else: ?>
<form method="POST" action="<?php echo e(route('admin.themes.activate')); ?>" class="inline">
<?php echo csrf_field(); ?>
<input type="hidden" name="theme" value="<?php echo e($theme['slug']); ?>">
...
<form method="POST" action="<?php echo e(route('admin.themes.settings')); ?>">
<?php echo csrf_field(); ?>
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
<div>
<label class="text-slate-800 dark:text-white text-sm font-medium mb-2 block">Site Color</label>
<div class="flex items-center gap-3">
<input type="color" name="site_color" value="<?php echo e($settings['site_color'] ?? '#19bc9c'); ?>" class="w-12 h-8 border border-slate-300 dark:border-[#324d67] rounded cursor-pointer">
<input type="text" name="site_color_text" value="<?php echo e($settings['site_color'] ?? '#19bc9c'); ?>" class="flex-1 border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white" readonly>
</div>
</div>
...
<div class="flex items-center gap-3">
<input type="color" name="header_color" value="<?php echo e($settings['header_color'] ?? '#161616'); ?>" class="w-12 h-8 border border-slate-300 dark:border-[#324d67] rounded cursor-pointer">
<input type="text" name="header_color_text" value="<?php echo e($settings['header_color'] ?? '#161616'); ?>" class="flex-1 border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white" readonly>
</div>
</div>
</div>
...
document.getElementById('darkModeToggle').addEventListener('change', function() {
    ...
    fetch('<?php echo e(route("admin.themes.dark-mode")); ?>', {
        method: 'POST',
        headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Laravel Projects\Multi Author Blog\resources\views\admin\themes\index.blade.php ENDPATH**/ ?>