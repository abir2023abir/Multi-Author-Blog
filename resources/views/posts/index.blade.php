...
@section('title', 'All Posts - Multi Author Blog')
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-3/4">
                ...
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 mb-12">
                    ...
                    <form method="GET" action="{{ route('posts.index') }}" id="filterForm" class="space-y-6">
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                            ...
                            <a href="{{ route('posts.index') }}" class="text-gray-500 hover:text-gray-700 transition-colors flex items-center space-x-2">
                    ...
                    @forelse($posts as $post)
                        <article class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-200 transform hover:-translate-y-1">
                            ...
                            <div class="p-6">
                                ...
                                <p class="text-gray-700 mb-6 line-clamp-3 leading-relaxed">{{ Str::limit(strip_tags($post->content), 120) }}</p>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-800">{{ $post->user->name }}</p>
            ...
            <div class="lg:w-1/4 space-y-8">
                            ...
                            <div class="group flex space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-semibold text-gray-800 line-clamp-2 group-hover:text-blue-600 transition-colors">
                                        <a href="{{ route('posts.show', $recentPost) }}" class="hover:no-underline">
                                            {{ $recentPost->title }}
                                        </a>
                                    </h4>
                                    <div class="flex items-center space-x-2 mt-1">
                                        <span class="text-xs text-gray-500">{{ $recentPost->user->name }}</span>
                                        <span class="text-xs text-gray-400">•</span>
                                        <span class="text-xs text-gray-500">{{ $recentPost->published_at->format('M d') }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="space-y-4">
                        @foreach($popularPosts as $index => $popularPost)
                            <div class="group flex space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-semibold text-gray-800 line-clamp-2 group-hover:text-blue-600 transition-colors">
                                        <a href="{{ route('posts.show', $popularPost) }}" class="hover:no-underline">
                                            {{ $popularPost->title }}
                                        </a>
                                    </h4>
                                    <div class="flex items-center space-x-2 mt-1">
                                        <span class="text-xs text-gray-500 flex items-center">
                                            ...
                                            {{ $popularPost->view_count ?? 0 }}
                                        </span>
                                        <span class="text-xs text-gray-400">•</span>
                                        <span class="text-xs text-gray-500">{{ $popularPost->published_at->format('M d') }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                    ...
                    <div class="space-y-2">
                        @foreach($categories as $category)
                            <a href="{{ route('posts.index', ['category' => $category->name]) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        ...
                        <li><a href="{{ route('posts.index') }}" class="text-gray-300 hover:text-white transition-colors text-sm">All Posts</a></li>
                    ...
                    <ul class="space-y-2">
                        @if($categories->count() > 0)
                            @foreach($categories->take(5) as $category)
                                <li><a href="{{ route('posts.index', ['category' => $category->name]) }}" class="text-gray-300 hover:text-white transition-colors text-sm">{{ $category->name }}</a></li>
                            @endforeach
                        @else
            ...
            <div class="border-t border-gray-800 mt-8 pt-6">
                <div class="flex flex-col md:flex-row justify-between items-center text-sm">
                    <div class="text-gray-400 mb-2 md:mb-0">
                        © {{ date('Y') }} Multi Author Blog. All rights reserved.
                    </div>
