...
<p class="text-slate-800 dark:text-white tracking-light text-2xl font-bold leading-tight">{{ $totalFiles }}</p>
</div>
<div class="rounded-lg border border-slate-200 dark:border-[#324d67] bg-white dark:bg-[#111a22] p-6">
<p class="text-slate-600 dark:text-white text-base font-medium leading-normal">Total Size</p>
<p class="text-slate-800 dark:text-white tracking-light text-2xl font-bold leading-tight">{{ $totalSize }} MB</p>
</div>
<div class="rounded-lg border border-slate-200 dark:border-[#324d67] bg-white dark:bg-[#111a22] p-6">
<form method="POST" action="{{ route('admin.storage.clean') }}">
@csrf
