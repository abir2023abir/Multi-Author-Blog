...
<!-- Page Header -->
<div class="flex items-center justify-between mb-6">
<h1 class="text-slate-800 dark:text-white text-3xl font-bold">Recommended Posts</h1>
...
<div class="w-10 h-10 bg-blue-200 dark:bg-blue-800 rounded"></div>
<div>
...
@elseif($post->status === 'draft') bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300
@else bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300
@endif">
{{ ucfirst($post->status) }}
</span>
</td>
<td class="px-4 py-3 text-slate-600 dark:text-white text-sm">{{ $post->published_at ? $post->published_at->format('M d, Y') : '-' }}</td>
<td class="px-4 py-3">
...
@empty
<tr>
<td colspan="7" class="px-4 py-8 text-center text-slate-500 dark:text-[#92adc9]">No recommended posts found.</td>
</tr>
@endforelse
</tbody>
</table>
