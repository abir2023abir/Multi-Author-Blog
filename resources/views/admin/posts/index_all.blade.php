...
<a class="text-primary hover:underline" href="{{ route('posts.show', $post) }}">{{ Str::limit($post->title, 40) }}</a>
</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ $post->user->name }}</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ optional($post->category)->name ?? 'Uncategorized' }}</td>
...
<form method="POST" action="{{ route('admin.posts.toggle-type', $post) }}" class="inline">
@csrf
<input type="hidden" name="type" value="featured">
<button type="submit" class="px-2 py-1 text-xs rounded {{ $post->is_featured ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' }}">
⭐ Featured
</button>
</form>
...
<button type="submit" class="px-2 py-1 text-xs rounded {{ $post->is_slider ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' }}">
🎠 Slider
</button>
</form>
...
<button type="submit" class="px-2 py-1 text-xs rounded {{ $post->is_breaking ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' }}">
🚨 Breaking
</button>
</form>
...
<button type="submit" class="px-2 py-1 text-xs rounded {{ $post->is_recommended ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' }}">
👍 Recommended
</button>
</form>
</div>
</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ optional($post->published_at)->format('Y-m-d') ?? 'Not published' }}</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
