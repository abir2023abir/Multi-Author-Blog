...
<a href="{{ route('admin.ads.create') }}" class="flex items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em]">
...
<td class="h-[72px] px-4 py-2 text-slate-800 dark:text-white text-sm font-normal leading-normal">{{ $ad->name }}</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ $ad->position }}</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
<span class="inline-flex items-center justify-center rounded-full {{ $ad->is_active ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300' }} px-2.5 py-0.5 text-xs font-medium">{{ $ad->is_active ? 'Active' : 'Inactive' }}</span>
</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
<div class="flex gap-2">
<a href="{{ route('admin.ads.edit', $ad) }}" class="text-xs font-medium text-primary hover:text-primary/80">Edit</a>
