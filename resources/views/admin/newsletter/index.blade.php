...
<a href="{{ route('admin.newsletter.create') }}" class="flex items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em]">
...
<td class="h-[72px] px-4 py-2 text-slate-800 dark:text-white text-sm font-normal leading-normal">{{ $subscriber->email }}</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
<span class="inline-flex items-center justify-center rounded-full {{ $subscriber->is_subscribed ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300' }} px-2.5 py-0.5 text-xs font-medium">{{ $subscriber->is_subscribed ? 'Subscribed' : 'Unsubscribed' }}</span>
</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ $subscriber->created_at->format('Y-m-d') }}</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
