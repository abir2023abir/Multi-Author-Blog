...
<input type="text" name="title" value="{{ old('title', $page->title) }}" class="w-full border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white @error('title') border-red-500 @enderror" placeholder="Enter page title">
@error('title')
...
<input type="text" name="slug" value="{{ old('slug', $page->slug) }}" class="w-full border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white @error('slug') border-red-500 @enderror" placeholder="page-url-slug">
@error('slug')
<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
@enderror
...
<textarea name="content" rows="15" class="w-full border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white @error('content') border-red-500 @enderror" placeholder="Enter page content">{{ old('content', $page->content) }}</textarea>
@error('content')
<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
@enderror
</div>
...
<textarea name="meta_description" rows="3" class="w-full border border-slate-300 dark:border-[#324d67] rounded px-3 py-2 bg-white dark:bg-[#111a22] text-slate-800 dark:text-white @error('meta_description') border-red-500 @enderror" placeholder="Brief description for search engines">{{ old('meta_description', $page->meta_description) }}</textarea>
...
<option value="main_menu" {{ old('location', $page->location ?? 'main_menu') == 'main_menu' ? 'selected' : '' }}>Main Menu</option>
<option value="footer" {{ old('location', $page->location) == 'footer' ? 'selected' : '' }}>Footer</option>
<option value="top_menu" {{ old('location', $page->location) == 'top_menu' ? 'selected' : '' }}>Top Menu</option>
</select>
@error('location')
<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
@enderror
...
<option value="custom" {{ old('page_type', $page->page_type ?? 'custom') == 'custom' ? 'selected' : '' }}>Custom</option>
...
<option value="1" {{ old('is_published', $page->is_published) == '1' ? 'selected' : '' }}>Published</option>
<option value="0" {{ old('is_published', $page->is_published) == '0' ? 'selected' : '' }}>Draft</option>
</select>
@error('is_published')
<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
@enderror
...
<p class="text-slate-500 dark:text-[#92adc9] text-xs mb-2">Created: {{ $page->created_at->format('M d, Y') }}</p>
<p class="text-slate-500 dark:text-[#92adc9] text-xs mb-2">Last Updated: {{ $page->updated_at->format('M d, Y') }}</p>
