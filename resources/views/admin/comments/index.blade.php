...
<a class="text-primary hover:underline" href="{{ route('posts.show', $c->post) }}">{{ Str::limit($c->post->title, 30) }}</a>
</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ optional($c->user)->name ?? 'Guest' }}</td>
<td class="h-[72px] px-4 py-2 text-slate-500 dark:text-[#92adc9] text-sm font-normal leading-normal">{{ Str::limit($c->content, 80) }}</td>
<td class="h-[72px] px-4 py-2 text-sm font-normal leading-normal">
<div class="flex gap-2">
<form method="POST" action="{{ route('admin.comments.destroy', $c) }}" class="inline">
@csrf
@method('DELETE')
